...
if __name__ == '__main__':
    try:
        img = cv2.imread('dolphin.jpg')     #loading the image
        imgray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  #converting image into grayscale
        ret, thresh = cv2.threshold(imgray, 127, 255, 0) #converting grayscale image into binary image
        contours, hierarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE) #retrieves contours from the binary image
        ...
        r=0
        s=0
        x,y=[],[]  #defining x,y coordinates list
